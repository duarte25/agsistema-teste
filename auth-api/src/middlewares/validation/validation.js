...
export class Validator {
    ...
    getErrors() {
        ...
        if (errosFiltrados.length > 0) {
            return errosFiltrados.map((path) => this.validations[path].error);
        } else {
            return [];
        }
    }
...
export class ValidationFuncs {
    ...
    */
     ...
     */
    static unique = (opcoes = { model: false, query: false, ignoreSelf: false }) => async (value, val) => {
        ...
        // Se ignorar o próprio usuário, adiciona o critério de comparação para ignorar o ID do usuário atual
        if (opcoes.ignoreSelf && opcoes.userId) {
            query._id = { $ne: opcoes.userId };  // O $ne (not equal) irá garantir que o próprio ID seja ignorado
        }
    
        // Procura um usuário que já tenha o mesmo valor
        let resultado = await opcoes.model.findOne(query);
    
        // Se o resultado for encontrado, retorna uma mensagem de erro
        if (resultado) {
            return opcoes.message || messages.validationGeneric.fieldIsRepeated(val.path).message;
        }
    
        // Se não encontrar, o valor é único
        return true;
    };
    ...
    static passwordComplexity = (opcoes = {}) => ValidationFuncs.regex({
